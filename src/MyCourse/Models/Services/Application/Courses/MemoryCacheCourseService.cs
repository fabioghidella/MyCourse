...
public class MemoryCacheCourseService : ICachedCourseService
{
    ...
    public Task<CourseDetailViewModel> GetCourseAsync(int id)
    {
        ...
        {
            cacheEntry.SetAbsoluteExpiration(TimeSpan.FromSeconds(60)); //Exercise: retrieve the 60s value from the configuration service
    ...
    {
        //Cache results only for the first 5 pages of the catalog, which are expected to be
        //the most visited, giving the greatest benefit from caching.
        //Also, results are only cached when the user has not performed a search.
        //This drastically reduces RAM consumption.
        bool canCache = model.Page <= 5 && string.IsNullOrEmpty(model.Search);

        //If canCache is true, use the caching mechanism
        if (canCache)
        {
        ...
        //Otherwise fall back to the underlying application service, which always fetches values from the database
        return courseService.GetCoursesAsync(model);
    }
